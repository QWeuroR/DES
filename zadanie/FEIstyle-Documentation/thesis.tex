...
\FEIauthor{Bc. Jakub Janega}

% Evidence number
\FEIregNr{FEI-xxxx-xxxx}

% Title
\FEItitle{Semestrálny projekt}
...
\FEIcourseTag{I-DES}         % Course code
\FEIcourse{Dynamika elektrizačných sústav}       % Course name in Slovak
\FEIcourseEn{Course name}        % Course name in English
\FEIlecturer{doc. Ing. Martin Ernek, PhD.}    % Lecturer's name
\FEITA{doc. Ing. Martin Ernek, PhD.}           % Teaching assistant's name
...
\FEItitlePage
\includepdf[pages=-]{includes/zadanie_Janega_Jakub.pdf} % turned on for documentation type - QWeuroR
